...
This script will munge all data and produce an output file for processing. Note that much of this set of mechanics will be different in the full analysis, because we are now asking for data in a particular template format. So the preprocessing scripts will deal with getting from idiosynratic formats to *that* format.
...
Some items to be fixed in the real data analysis:
...
```{r}
gervain_data <- suppressWarnings(read_csv("pilot/gervain/Trials.csv", 
                                           col_types = cols())) %>%
  left_join(suppressWarnings(read_csv("pilot/gervain/Participants.csv", 
                                           col_types = cols()))) %>%
  mutate(lab = "lpp-paris", 
         method = "HPP",
         trial_num = floor((trial_num+1)/2)) %>%
  select(lab, method, subid, age_days, trial_type, trial_num, looking_time)
```
...
```{r}
d <- bind_rows(floccia_data, hamlin_data, frank_data, gonzalez_data, gervain_data)
kable(head(d))
```
